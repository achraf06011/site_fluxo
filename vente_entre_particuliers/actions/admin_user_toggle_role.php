...
if ($id === currentUserId()) {
  $_SESSION["flash_error"] = "Tu ne peux pas changer ton propre rôle.";
  header("Location: ../admin/users.php");
  exit;
}

try {
  $stmt = $pdo->prepare("SELECT role FROM user WHERE id_user = ? LIMIT 1");
  $stmt->execute([$id]);
  $u = $stmt->fetch();
  if (!$u) {
  ...
  $_SESSION["flash_success"] = "Rôle utilisateur mis à jour : " . $new;
  header("Location: ../admin/users.php");
  exit;
